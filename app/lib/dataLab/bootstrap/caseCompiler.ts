...
function allowedFocus(phase: number, challenge: string) {
  const map: Record<number, string[]> = {
    ...
    2: [{ challenge: '假设缺失', focus: 'expected_result' },
      ...
      { challenge: '测量方式含糊', focus: 'data_recording' },
      { challenge: '控制变量混乱', focus: 'experiment_process' },
      { challenge: '材料缺失', focus: 'experiment_process' },
      { challenge: '步骤缺失', focus: 'experiment_process' },
      { challenge: '重复次数缺失', focus: 'experiment_process' },
      { challenge: '安全异常', focus: 'experiment_process' },
...
function focusDescriptions(focusIds: string[]) {
  const descriptions: Record<string, string> = {
    ...
    variable_design: '只推进变量设计环节：唯一自变量、至少两个可比较水平和观测指标；同一条消息中已说清的内容全部接收',
    data_recording: '只推进数据记录环节：测量工具、读数时间和原始记录字段',
    experiment_process: '只推进实验过程环节：具体步骤、控制条件和独立重复轮数，并区分每组样本数',
    expected_result: '只推进结果推测环节：请学生自己说明趋势，不替学生给出假设',
