...
export default function WorkloadReviewTable({ people, items }: { people: PersonSummary[]; items: WorkItem[] }) {
  ...
  async function review(item: WorkItem, nextStatus: Exclude<WorkReviewStatus, 'PENDING'>) {
    ...
    const invalidEffect = item.campaign.status === 'ACTIVE' ? '重新进入公共队列' : '保持取消且不会重新分发';
    if (nextStatus === 'INVALID' && !window.confirm(`判定无效后，该任务会${invalidEffect}，原参与者不计有效条数。确定继续吗？`)) return;
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-3">
      ...
      {filtered.length === 0 ? <div className="rounded-xl border bg-white p-8 text-center text-sm text-gray-500">当前筛选条件下没有记录。</div> : <div className="space-y-3">{filtered.map((item) => { const errors = item.check.issues.filter((checkIssue) => checkIssue.severity === 'error'); const warnings = item.check.issues.filter((checkIssue) => checkIssue.severity === 'warning'); return <article key={item.id} className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-1 text-xs ${badgeClass[item.status]}`}>{WORK_REVIEW_LABELS[item.status]}</span><span className="text-xs text-blue-700">阶段 {item.phase}</span><span className={`rounded-full px-2 py-1 text-xs ${errors.length > 0 ? 'bg-red-100 text-red-700' : warnings.length > 0 ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-700'}`}>{errors.length > 0 ? `${errors.length} 个自动错误` : warnings.length > 0 ? `${warnings.length} 个复核项` : '自动检查通过'}</span><span className="text-xs text-gray-500">{item.campaign.name}</span>{item.campaign.status === 'ARCHIVED' && <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-600">活动已归档</span>}</div><h3 className="mt-2 font-medium">{item.scenario}</h3><p className="mt-1 break-all text-xs text-gray-500">{item.sourceRecordId}</p></div><div className="text-right text-sm"><div className="font-medium">{item.participant.displayName}</div><div className="text-xs text-gray-500">{item.participant.username} · {new Date(item.submittedAt).toLocaleString('zh-CN')}</div></div></div><details className="mt-3 rounded-lg bg-gray-50 p-3"><summary className="cursor-pointer text-sm font-medium">查看本次提交内容与自动检查</summary><div className="mt-3 space-y-2">{item.preview.map((text, index) => <p key={index} className="whitespace-pre-wrap border-l-2 border-gray-300 pl-3 text-sm leading-6 text-gray-700">{text}</p>)}</div>{item.check.issues.length > 0 && <ul className="mt-3 space-y-1 border-t pt-3 text-xs">{item.check.issues.map((checkIssue) => <li key={`${checkIssue.ruleCode}-${checkIssue.messageIndex}-${checkIssue.message}`} className={checkIssue.severity === 'error' ? 'text-red-700' : 'text-amber-800'}>• [{checkIssue.severity === 'error' ? '错误' : '人工复核'}] {checkIssue.message}</li>)}</ul>}</details>{item.status === 'PENDING' ? <div className="mt-3 grid gap-2 md:grid-cols-[1fr_auto]"><input value={notes[item.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="审核说明（退回或无效时必填）" className="rounded-lg border px-3 py-2 text-sm" /><div className="flex flex-wrap gap-2"><button onClick={() => review(item, 'APPROVED')} disabled={pendingId === item.id || errors.length > 0} title={errors.length > 0 ? '存在硬错误，不能计为有效标注' : '通过并计入有效条数'} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white disabled:opacity-50">通过并计 1 条</button><button title={item.campaign.status === 'ACTIVE' ? '退回原参与者修改' : '活动已归档，不能重新开放任务'} onClick={() => review(item, 'RETURNED')} disabled={pendingId === item.id || item.campaign.status !== 'ACTIVE'} className="rounded-lg border border-blue-200 px-3 py-2 text-sm text-blue-700 disabled:cursor-not-allowed disabled:opacity-35">退回修改</button><button onClick={() => review(item, 'INVALID')} disabled={pendingId === item.id} className="rounded-lg border border-red-200 px-3 py-2 text-sm text-red-700 disabled:opacity-50">标记无效</button></div></div> : <div className="mt-3 text-xs text-gray-600">审核人：{item.reviewer?.displayName ?? '-'}{item.note ? ` · ${item.note}` : ''}</div>}</article>; })}</div>}
    </section>
  </div>;
}
