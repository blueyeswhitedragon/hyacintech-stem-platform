...
export default function ReviewWorkbench() {
  ...
  return <div className="space-y-5">
    <div className="border bg-white p-4"><div className="text-xs font-medium text-blue-700">P{item.phase} · 匿名仲裁</div><h2 className="mt-1 text-lg font-semibold">{item.scenario}</h2>{item.campaignStatus === 'ARCHIVED' && <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs leading-5 text-slate-700">所属活动已经归档。你仍可接受、合并或拒绝已有版本，但不能再退回给标注员修改。</div>}{stylePolicy && <div className="mt-3 rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><div className="font-medium">共同目标风格：{stylePolicy.label}</div><p className="mt-1 text-xs leading-5">{stylePolicy.summary} 判定时同时检查：{stylePolicy.annotationRubric.join('；')}。</p></div>}{item.styleTargetMismatch && <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">这是旧活动遗留任务，候选版本的目标风格不一致。本次只能按内容、阶段和结构质量仲裁，不能用于判断风格遵循度。</div>}<div className="mt-3 max-w-4xl space-y-2">{item.original.conversations.filter((entry) => entry.from === 'human').map((entry, index) => <p key={index} className="border-l-4 border-gray-300 pl-3 text-sm leading-6">{entry.value}</p>)}</div>{item.autoCheck?.issues?.length > 0 && <div className="mt-3 border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">原始样本自动检查：{item.autoCheck.issues.map((check) => check.message).join('；')}</div>}</div>
    <div className="grid gap-4 xl:grid-cols-2">{item.candidates.map((candidate) => {
      ...
      return <label key={candidate.id} className={`block cursor-pointer border bg-white p-4 ${selected === candidate.id ? 'border-blue-600 ring-1 ring-blue-600' : ''}`}><div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="text-lg font-semibold">版本 {candidate.label}</span><span className={`rounded-full px-2 py-1 text-xs ${errors.length > 0 ? 'bg-red-100 text-red-700' : warnings.length > 0 ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-700'}`}>{errors.length > 0 ? `${errors.length} 个错误 · ${warnings.length} 个复核项` : warnings.length > 0 ? `${warnings.length} 个复核项 · 可选择` : '自动检查通过'}</span></div><input type="radio" name="candidate" value={candidate.id} checked={selected === candidate.id} onChange={() => setSelected(candidate.id)} /></div>{generalIssues.length > 0 && <ul className="mt-3 space-y-1 rounded border border-amber-200 bg-amber-50 p-2 text-xs">{generalIssues.map((checkIssue) => <li key={`${checkIssue.ruleCode}-${checkIssue.message}`} className={checkIssue.severity === 'error' ? 'text-red-700' : 'text-amber-800'}>• [{checkIssue.severity === 'error' ? '错误' : '人工复核'}] {checkIssue.message}</li>)}</ul>}<div className="mt-4 space-y-4">{details.map((turn, turnIndex) => { const turnErrors = turn.issues.filter((checkIssue) => checkIssue.severity === 'error'); const turnWarnings = turn.issues.filter((checkIssue) => checkIssue.severity === 'warning'); return <div key={turn.index} className={`rounded border p-3 ${turnErrors.length > 0 ? 'border-red-200 bg-red-50' : turnWarnings.length > 0 ? 'border-amber-200 bg-amber-50' : 'border-gray-200'}`}><div className="mb-2 flex flex-wrap gap-2 text-xs"><span className="font-medium">导师回复 {turnIndex + 1}</span>{turn.response && <><span className="rounded bg-blue-50 px-2 py-0.5 text-blue-700">action: {turn.response.next_action_type}</span><span className="rounded bg-gray-100 px-2 py-0.5">phase_complete: {String(turn.response.phase_complete)}</span><span className="rounded bg-purple-50 px-2 py-0.5 text-purple-700">schema: {turn.response.data_table_schema?.columns.length ?? 0} 列</span></>}</div><p className="whitespace-pre-wrap text-sm leading-6 text-gray-800">{turn.response?.dialogue ?? turn.raw}</p>{turn.response?.data_table_schema && <p className="mt-2 text-xs text-gray-500">字段：{turn.response.data_table_schema.columns.map((column) => `${column.key}(${column.type})`).join('、')}</p>}{turn.issues.length > 0 && <ul className="mt-2 space-y-1 text-xs">{turn.issues.map((checkIssue) => <li key={`${checkIssue.ruleCode}-${checkIssue.message}`} className={checkIssue.severity === 'error' ? 'text-red-700' : 'text-amber-800'}>• [{checkIssue.severity === 'error' ? '错误' : '人工复核'}] {checkIssue.message}</li>)}</ul>}</div>; })}</div></label>;
    })}</div>
    <div className="grid gap-4 border bg-white p-4 lg:grid-cols-[220px_1fr_auto]"><label className="text-sm">最终等级<select value={tier} onChange={(event) => setTier(event.target.value as typeof tier)} className="mt-1 w-full border px-2 py-2"><option value="human_gold">Human Gold</option><option value="reviewed_silver">Reviewed Silver</option><option value="reject">Reject</option></select></label><label className="text-sm">仲裁理由<textarea value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1 min-h-20 w-full border p-2" /></label><div className="flex flex-wrap items-end gap-2"><button onClick={() => decide('SELECT')} disabled={pending || selectedHasErrors} className="bg-gray-950 px-3 py-2 text-sm text-white disabled:opacity-40">接受所选</button><button title={item.campaignStatus === 'ACTIVE' ? '退回标注员修改' : '活动已归档，不能重新开放任务'} onClick={() => decide('RETURN')} disabled={pending || item.campaignStatus !== 'ACTIVE'} className="border px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-35">退回</button><button onClick={() => decide('REJECT')} disabled={pending} className="border border-red-600 px-3 py-2 text-sm text-red-700">拒绝</button></div></div>{message && <p className="text-sm text-red-600">{message}</p>}
  </div>;
}
