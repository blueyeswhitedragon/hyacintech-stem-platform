...
export default function RuntimeBundleManager({
  ...
  return <div className="space-y-6">
    <section className="border border-hairline bg-canvas p-5"><h2 className="font-semibold">角色默认预选</h2><p className="mt-1 text-sm text-muted">默认绑定只为后续 Data Lab 批次或部署表单提供预选，不会绕过评测门禁、创建部署或切换任何会话。</p><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{roles.map((role) => <div key={role.roleKey} className="border border-hairline bg-surface-soft p-3"><div className="flex justify-between gap-2"><b className="text-sm">{role.displayName}</b><span className="text-xs text-muted">{role.enabled ? '启用' : '停用'}</span></div><p className="mt-1 min-h-10 text-xs leading-5 text-muted">{role.description}</p><label className="mt-2 block text-xs">后续表单默认组合<Select value={role.defaultRuntimeBundle?.id ?? ''} onChange={(event) => { const bundle = bundles.find((item) => item.id === event.target.value); if (bundle) setDefaultTarget({ role, bundle }); }} className="mt-1"><option value="">尚未绑定</option>{bundles.filter((bundle) => bundle.roleKey === role.roleKey && ['AVAILABLE', 'DEPLOYED'].includes(bundle.status)).map((bundle) => <option key={bundle.id} value={bundle.id}>{bundle.name} v{bundle.version}</option>)}</Select></label></div>)}</div></section>
