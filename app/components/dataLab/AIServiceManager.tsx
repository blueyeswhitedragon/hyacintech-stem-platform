...
export default function AIServiceManager({
  ...
  async function test(connection: ConnectionItem, showModels = false) {
    const data = await call(`/api/data-lab/ai-services/${connection.id}/test`, { method: 'POST' }, '模型列表与 JSON 模式生成探针均通过，服务已启用。');
  ...
  return <div className="space-y-6">
    <section className="flex flex-wrap items-start justify-between gap-4 border border-hairline bg-canvas p-5">
      <div><h2 className="font-semibold">服务连接</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-muted">先登记 Base URL 与安全凭据，再测试模型列表和一次最小 JSON 模式生成，最后登记远程 model ID。点击测试会向所选服务发起一次很小的生成请求；密钥不会在页面、API 响应或日志中明文返回。</p></div>
    ...
    {GROUPS.map(([status, label]) => <section key={status} className="space-y-3">
      ...
      {(grouped[status] ?? []).map((connection) => <article key={connection.id} className="border border-hairline bg-canvas p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><div className="text-xs text-muted">{connection.protocol} · {connection.credential?.sourceType === 'ENV' ? `环境变量 ${connection.credential.envVarName}` : '数据库加密凭据'}{connection.credential?.keyLastFour ? ` · 末四位 ${connection.credential.keyLastFour}` : ''}</div><h3 className="mt-1 font-semibold">{connection.name}</h3><p className="mt-1 break-all text-sm text-body">{connection.baseUrl}</p></div>
          <span className={`px-2 py-1 text-xs ${status === 'ACTIVE' ? 'bg-success/10 text-body-strong' : status === 'ERROR' ? 'bg-error/10 text-body-strong' : 'bg-surface-card'}`}>{label}</span>
        </div>
        <div className="mt-4 grid gap-3 text-xs sm:grid-cols-3">
          <div className="border border-hairline bg-surface-soft p-3"><b>最近连接测试</b><p className="mt-1 text-muted">{connection.lastTestedAt ? new Date(connection.lastTestedAt).toLocaleString('zh-CN') : '尚未测试'}{connection.lastLatencyMs !== null ? ` · ${connection.lastLatencyMs} ms` : ''}</p></div>
          <div className="border border-hairline bg-surface-soft p-3"><b>Endpoint</b><p className="mt-1 text-muted">{connection.endpointCount} 个已登记</p></div>
          <div className="border border-hairline bg-surface-soft p-3"><b>运行组合引用</b><p className="mt-1 text-muted">{connection.runtimeBundleCount} 个</p></div>
        </div>
        {connection.lastErrorMessage && <div className="mt-3 border border-error/40 bg-error/8 p-3 text-xs text-body-strong"><b>为什么不能继续：</b>{connection.lastErrorMessage}<p className="mt-1">修复路径：核对连接信息与访问密钥，再重新测试服务可用性。</p></div>}
        {connection.endpoints.length > 0 && <details className="mt-3 border border-hairline p-3"><summary className="cursor-pointer text-sm font-medium">查看已登记模型（{connection.endpoints.length}）</summary><div className="mt-3 space-y-2">{connection.endpoints.map((endpoint) => <div key={endpoint.id} className="flex flex-wrap items-center justify-between gap-2 bg-surface-soft p-2 text-xs"><span><b>{endpoint.displayName}</b> · {endpoint.remoteModelId}{endpoint.modelVersionId ? ' · 已关联模型产物' : ' · 尚未关联模型产物'}</span><span>{endpoint.runtimeBundleCount} 个组合引用</span></div>)}</div></details>}
        {availableModels[connection.id] && <div className="mt-3 border border-info/40 bg-info/8 p-3 text-xs text-body-strong"><b>服务返回的可用 model ID：</b><p className="mt-1 break-words">{availableModels[connection.id].join('、') || '服务可访问，但没有返回模型列表；可手工登记 model ID。'}</p><p className="mt-1">下一步：将需要在平台使用的 model ID 登记为 Endpoint。</p></div>}
        <div className="mt-4 flex flex-wrap gap-2">
          <button disabled={pending || status === 'DISABLED'} onClick={() => test(connection)} className={buttonClass('secondary', 'sm')}>测试连接</button>
          <button disabled={pending || status === 'DISABLED'} onClick={() => test(connection, true)} className={buttonClass('secondary', 'sm')}>查看可用模型</button>
          <button onClick={() => setEditing(connection)} className={buttonClass('secondary', 'sm')}>编辑连接信息</button>
          <button onClick={() => setCredentialTarget(connection)} className={buttonClass('secondary', 'sm')}>更新访问密钥</button>
          <button disabled={status !== 'ACTIVE'} onClick={() => setEndpointTarget(connection)} className={buttonClass('secondary', 'sm')}>关联可用模型</button>
          {status !== 'DISABLED' ? <button onClick={() => stateAction(connection, 'DISABLE')} className={buttonClass('danger', 'sm')}>停用服务连接</button> : <button onClick={() => stateAction(connection, 'ENABLE')} className={buttonClass('primary', 'sm')}>重新启用</button>}
          {connection.endpointCount === 0 && <button onClick={() => setDeleteTarget(connection)} className={buttonClass('danger', 'sm')}>删除未使用连接</button>}
        </div>
      </article>)}
      {(grouped[status]?.length ?? 0) === 0 && <p className="border border-hairline bg-canvas p-4 text-sm text-muted">当前没有{label}的服务连接。</p>}
    </section>)}
    ...
    <Dialog open={Boolean(endpointTarget)} title="关联可用模型" description="登记远程 model ID，并可选关联一个不可变模型产物。" onClose={() => !pending && setEndpointTarget(null)}>
      <form action={createEndpoint} className="space-y-3">
        <label className="block text-sm">Endpoint 显示名称<Input name="displayName" required placeholder="Qwen STEM 推理端点" className="mt-1" /></label>
        <label className="block text-sm">远程 model ID<Input name="remoteModelId" required placeholder="Qwen3.5-35B-A3B" className="mt-1" /></label>
        <label className="block text-sm">关联模型产物（可稍后设置）<Select name="modelVersionId" className="mt-1"><option value="">暂不关联</option>{models.map((model) => <option key={model.id} value={model.id}>{model.tag}</option>)}</Select></label>
        <button disabled={pending} className={buttonClass('primary', 'md')}>登记模型 Endpoint</button>
      </form>
    </Dialog>
