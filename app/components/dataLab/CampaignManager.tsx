...
export default function CampaignManager({ batches, annotators }: { batches: Array<{ id: string; name: string }>; annotators: AnnotatorOption[] }) {
  ...
  return <form action={create} className="space-y-5 rounded-xl border bg-white p-4 shadow-sm md:p-5">
    ...
    <details className="rounded-xl border bg-gray-50 p-4"><summary className="cursor-pointer text-sm font-medium">高级选项：阶段、候选等级和回复风格</summary><div className="mt-4 space-y-4"><fieldset><legend className="text-sm font-medium">实验阶段</legend><div className="mt-2 flex flex-wrap gap-2">{[1, 2, 3, 4, 5, 6].map((phase) => <label key={phase} className="rounded-lg border bg-white px-3 py-2 text-sm"><input name="phases" type="checkbox" value={phase} defaultChecked className="mr-2" />{phase}. {PHASE_META[phase].label}</label>)}</div></fieldset><fieldset><legend className="text-sm font-medium">候选等级</legend><div className="mt-2 flex flex-wrap gap-2"><label className="rounded-lg border bg-white px-3 py-2 text-sm"><input name="tiers" type="checkbox" value="gold_candidate" defaultChecked className="mr-2" />重点候选</label><label className="rounded-lg border bg-white px-3 py-2 text-sm"><input name="tiers" type="checkbox" value="silver" defaultChecked className="mr-2" />普通候选</label></div></fieldset><fieldset><legend className="text-sm font-medium">回复风格轮换权重（0 表示不使用）</legend><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{STYLE_FAMILIES.map((style) => <label key={style} className="text-xs text-gray-600">{STYLE_LABELS[style]}<input name={`style_${style}`} type="number" min="0" defaultValue="1" className="mt-1 w-full rounded-lg border bg-white px-2 py-2 text-sm text-gray-900" /></label>)}</div></fieldset></div></details>
