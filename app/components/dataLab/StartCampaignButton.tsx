...
export default function StartCampaignButton({ id }: { id: string }) {
  ...
  return <div className="flex items-center gap-2"><button type="button" onClick={start} disabled={pending} className="border border-ink px-3 py-1 text-xs disabled:opacity-50">{pending ? '启动中…' : '启动'}</button>{error && <span className="text-xs text-error">{error}</span>}</div>;
}
