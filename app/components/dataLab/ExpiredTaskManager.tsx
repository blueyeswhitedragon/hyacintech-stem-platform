...
export default function ExpiredTaskManager({ tasks }: { tasks: ExpiredTaskItem[] }) {
  ...
  return <section className="space-y-3 rounded-lg border border-hairline bg-canvas p-4 shadow-sm">
    <div><h2 className="font-semibold">过期任务管理</h2><p className="mt-1 text-sm text-muted">原标注员重新进入工作台会自动续租。只有确认需要换人时才释放；释放会清除未提交草稿。</p></div>
    {tasks.length === 0 ? <p className="text-sm text-muted">当前没有过期任务。</p> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-b-hairline bg-surface-soft"><tr><th className="p-3">活动/阶段</th><th className="p-3">场景</th><th className="p-3">原标注员</th><th className="p-3">过期时间</th><th className="p-3">草稿</th><th className="p-3"></th></tr></thead><tbody>{tasks.map((task) => <tr key={task.id} className="border-b border-b-hairline last:border-0"><td className="p-3"><div>{task.campaignName}</div><div className="text-xs text-muted">阶段 {task.phase} · {PHASE_META[task.phase]?.label}</div></td><td className="max-w-sm p-3">{task.scenario}</td><td className="p-3">{task.assignedTo ? `${task.assignedTo.displayName}（${task.assignedTo.username}）` : '-'}</td><td className="p-3 tabular-nums">{task.leaseExpiresAt ? new Date(task.leaseExpiresAt).toLocaleString('zh-CN') : '-'}</td><td className="p-3">{task.hasDraft ? <span className="text-[#8a6a0f]">有，释放将清除</span> : '无'}</td><td className="p-3"><button type="button" disabled={pendingId === task.id} onClick={() => setConfirmingTask(task)} className={buttonClass('danger', 'sm')}>释放任务</button></td></tr>)}</tbody></table></div>}
