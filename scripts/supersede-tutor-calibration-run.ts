...
async function main() {
  ...
  const [run, admin] = await Promise.all([
    db.bootstrapGenerationRun.findUnique({ where: { id: runId }, include: { cases: { include: { finalizedTurn: true, reviewTasks: true } } } }),
    db.user.findFirst({ where: { username, role: 'admin', isActive: true } }),
  ]);
  if (!run || run.kind !== 'CASE_COMPILATION' || !run.parametersJson.includes('"profile":"CALIBRATION_12"')) throw new Error('run 不存在或不是 Calibration 12 编译 run');
  ...
  if (run.cases.some((item) => item.finalizedTurn || item.reviewTasks.some((task) => task.status === 'SUBMITTED'))) {
    throw new Error('已有最终记录或已提交审核的 Calibration run 不允许整体 supersede');
  }
  await db.$transaction([
    ...
    db.dataLabAuditLog.create({ data: {
      actorId: admin.id,
      action: 'TUTOR_CALIBRATION_RUN_SUPERSEDED',
      entityType: 'BootstrapGenerationRun',
      entityId: runId,
      payloadJson: JSON.stringify({ reason, cases: run.cases.length, reviewTasks: run.cases.reduce((sum, item) => sum + item.reviewTasks.length, 0) }),
    } }),
  ]);
  console.log(JSON.stringify({ runId, status: 'SUPERSEDED', cases: run.cases.length, reason }, null, 2));
}
